...
public class ServicoDAO {
    ...
    public ServicoTO save(ServicoTO servico) {
        String sql = "insert into tb_servico (nm_servico, id_projeto, st_servico, ar_servico, id_usuario) values (?, ?, ?, ?, ?)";
        try (PreparedStatement ps = ConnectionFactory.getConnection().prepareStatement(sql)) {
            ...
            ps.setLong(5, servico.getUsuario().getId());
    ...
    public ServicoTO update(ServicoTO servico) {
        String sql = "update tb_servico set nm_servico = ?, id_projeto = ?, st_servico = ?, ar_servico = ?, id_usuario = ? where id_servico = ?";
        try (PreparedStatement ps = ConnectionFactory.getConnection().prepareStatement(sql)) {
            ps.setString(1, servico.getNome());
            ps.setLong(2, servico.getProjeto().getId());
            ps.setString(3, servico.getStatusServico().toString());
